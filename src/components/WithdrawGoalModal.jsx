...
export default function WithdrawGoalModal({ isOpen, onClose, goal }) {
  ...
  useEffect(() => {
    if (isOpen && goal) {
      setAmount(String(goal.currentAmount || ''))
  ...
  const newCurrent = Math.max(0, goal.currentAmount - amountNum)

  function validate() {
    const e = {}
    if (!amount || isNaN(amountNum) || amountNum <= 0) e.amount = 'Inválido'
    if (amountNum > goal.currentAmount) e.amount = `Máx. ${fmt(goal.currentAmount)}`
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-800">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <p className="text-xs text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2">
            Ahorrado: <span className="font-semibold text-gray-700 dark:text-gray-200">{fmt(goal.currentAmount)}</span> de {fmt(goal.targetAmount)}
          </p>

          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Monto a retirar</label>
            <input
          ...
          <div>
            ...
            <input
              type="text" placeholder={`Retiro de ${goal.name}`}
          ...
          <p className="text-xs text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2">
            Nuevo total ahorrado: <span className="font-semibold text-gray-700 dark:text-gray-200">{fmt(newCurrent)}</span>
